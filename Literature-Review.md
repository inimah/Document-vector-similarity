...
| \[5]. (Weston, et al., 2014) | Hashtags (words) prediction based on context text (sentence), Averaging word embedding; CNN | <ul><li>generates WE with word2vec</li><li>use pre-trained WE on CNN</li><li>in WE feature extraction, vector summation &amp; average, TF-IDF weighting are used</li></ul> | <ul><li>Only skimming to know how the experiments generate sentence-vector representation</li><li>w.r.t. CNN not sure if the authors intention to use this NN architecture is because either: linear projection of input-first convolutional layer (shared weights), or ... because in their experiments, the authors used dense vector representation of WE while CNN relates more in sparse representation (?)</li></ul> |
